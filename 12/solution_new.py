...
class Area:
    ...
    def sides(self) -> int:
        ...
        return sum(len(Area.find_connected_plots({p for p in self.plots if p+d not in self.plots})) for d in Area.directions)
...
def parse_input(file_handle) -> list[Area]:
    ...
    return [a for s in plots_per_plant_type.values() for a in Area.find_connected_plots(s)]
